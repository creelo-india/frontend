            ...
            <ul className='category-list'>
                {categoriesData.map((category, categoryIndex) => (
                    ...
                    >
                        ...
                        <p>{category.name} <FontAwesomeIcon icon={faChevronDown} /></p>
                        {activeCategory === categoryIndex && (
                            <div className='dropdown-content'>
                                {category.subcategories.map((subcat, subIndex) => (
                                    ...
                                    >
                                        <a className={`${activeSubcategory === subIndex && 'active'}`} href={subcat.link || '#'}>{subcat.name} <FontAwesomeIcon icon={faChevronRight} /></a>
                                        {activeSubcategory === subIndex && (
                                            <div className='nested-dropdown-content'>
                                                {subcat.subcategories.map((nestedSubcat, nestedIndex) => (
                                                    <a href={nestedSubcat.link} key={nestedIndex}>{nestedSubcat.name}</a>
                                                ))}
                                            </div>
                                        )}
                                    </div>
